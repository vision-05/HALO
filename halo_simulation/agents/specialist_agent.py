...
class GridCarbonAgent(BaseAgent):
    """Broadcasts synthetic UK grid carbon intensity (gCO2/kWh)."""
    ...
    def _forecast(self, now: float) -> list[float]:
        out = []
        t = now
        for _ in range(4):
            t += 60.0
...
class WeatherAgent(BaseAgent):
    """Outdoor temperature with seasonal offset and rare heatwave / cold snap events."""
