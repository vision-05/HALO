...
def _build_default_density_profile(center_hour: float, width_hours: float = 1.5, grid_points: int = 144) -> dict[
    str, Any]:
...
def _ensure_density_profile(section: dict[str, Any], fallback_peak: float) -> dict[str, Any]:
    for key in ("kde_profile", "return_home_kde_profile", "movie_night_kde_profile", "dog_walk_evening_kde_profile",
                "grocery_kde_profile", "dance_class_kde_profile"):
...
def _integrate_density_window(profile: dict[str, Any], start_hour: float, window_hours: float,
