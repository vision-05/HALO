                ...
                if trigger_timer:
                    ...
                    if elapsed_time >= REQUIRED_TIME:
                        ...
                        if msg_sent != True:
                            ...
                            await self.send_msg("LanguageAgent", json.dumps({"action": "self_prompt", "source": self.name, "target": "Claude", "params": {"prompt": "Suspicious person detected outside by security camera. Take appropriate action to keep the house safe. "}})) #hardcoded remove this
                            msg_sent = True
                else:
